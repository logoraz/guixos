(define-module (files gubar  blocks network-manager)
...
(define* (network-manager #:key (ssid #f) (signal 5))
...
Optional #:signal N sets the SIGRTMIN offset (default: 5)."
  (let ((pid (getpid)))
    (spawn-fiber
     (lambda ()
       (let ((input (open-input-pipe "nmcli monitor")))
         (let loop ((line (get-line input)))
